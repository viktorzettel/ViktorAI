...
export const SpaceViewport: React.FC = () => {
  ...
  // Debug State for Background Image
  ...
  return (
    <div className="relative w-full h-full bg-black isolate overflow-hidden">
      ...
      {/* DEBUG: Status Label */}
      <div className="absolute top-20 left-4 z-[100] bg-white text-black font-bold font-mono px-2 py-1 border-2 border-red-600 pointer-events-none select-none">
        BG: {bgStatus}
      </div>

      {/* DEBUG: Test Gradient Background (Red to Blue) */}
      {/* This ensures we know if the container is visible at all. If the image loads, it should cover this. */}
      <div className="absolute inset-0 z-0 bg-gradient-to-b from-red-600 via-purple-600 to-blue-600" />

      {/* 
        BACKGROUND LAYER 
        Local High-Quality Milky Way Panorama
        Expects 'milkyway_bg.jpg' in the public/root folder.
      */}
      <div className="absolute inset-0 z-0 select-none pointer-events-none">
        <img 
          ...
          onLoad={() => {
            console.log("BACKGROUND_IMAGE_LOADED");
            setBgStatus('LOADED');
          }}
          onError={(e) => {
            console.log("BACKGROUND_IMAGE_ERROR");
            setBgStatus('ERROR');
            e.currentTarget.style.display = 'none';
          }}
        />

        {/* Softened Overlay - Very light to not obscure debug gradient, but enough to pop text */}
